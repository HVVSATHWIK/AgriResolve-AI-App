...
export class ConsolidatedAgent {
  async run(imageB64: string, language: string = 'en', mode: string = 'diagnosis'): Promise<AssessmentData> {
    ...
    const bioProspectorPrompt = `
            You are "Agri-Twin Bio-Prospector", an expert botanist and ethnobotanist.
            Your goal is to Identify plants(weeds, crops, flora) and reveal their HIDDEN VALUE(Medicinal, Bio - fertilizer, Commercial).
            
            CORE PRINCIPLES:
    1. ACCURATE IDENTIFICATION: Identify the plant species visible.If uncertain, say "Unknown".
            2. VALUE DISCOVERY: Focus on * beneficial * properties.Even "weeds" have uses(e.g., nitrogen fixing, medicinal, compost).
            3. IGNORE DISEASE: unless it makes the plant unsafe to use.Focus on the PLANT ITSELF, not its health.

      CONTEXT:
    - User Language: ${language} (Translate findings / uses).

            OUTPUT SCHEMA(Strict JSON):
    {
      "subjectValidation": { "valid_subject": true, "message": "Valid plant detected" },
      "visionEvidence": {
        "lesion_color": "N/A", "lesion_shape": "N/A", "texture": "Green/Healthy", "distribution": "N/A",
          "anomalies_detected": [], "raw_analysis": "Plant identification analysis",
            "leaf_regions": [], "attention_boxes": []
      },
      "leafAssessments": [],
        "uncertaintyFactors": { "lowImageQuality": false, "multipleLeaves": false, "visuallySimilarConditions": false, "other": [] },
      "quality": { "score": 1.0, "flags": ["OK"], "reasoning": "Clear image" },
      "healthyResult": { "score": 1.0, "arguments": ["Identified healthy specimen"], "evidence_refs": { } },
      "diseaseResult": { "score": 0.0, "arguments": [], "evidence_refs": { } },
      "arbitrationResult": {
        "decision": "Identified: [Plant Name]",
          "confidence": 0.9,
            "rationale": ["Distinctive leaf shape", "Flower pattern matches [Species]"]
      },
      "explanation": {
        "summary": "This is [Plant Name]. It is often considered a weed but has [Property].",
          "guidance": ["Harvest for compost", "Use roots for [Medicinal Purpose]"]
      },
      "bioProspectorResult": {
        "plant_name": "Common Name",
          "scientific_name": "Latin Name",
            "medicinal_uses": ["Ayurvedic use for joint pain", "Traditional fever remedy"],
              "commercial_uses": ["High nitrogen content for compost", "Fiber source"],
                "tips": ["Best harvested in early morning", "Dry leaves before use"]
      }
    }

    IMPORTANT:
    - If the plant is a common crop(Rice, Wheat), mention its specific variety or growth stage value if possible.
            - If it is a WEED, highlight WHY the farmer should NOT just burn it.
    `;

    const prompt = isBioProspector ? bioProspectorPrompt : diagnosisPrompt;
